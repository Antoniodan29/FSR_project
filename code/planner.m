...
if (choice==1)
    t_iniz = 0;
    ttot=20; %duration
    tdead=30; %dead time to evaluate the steady-state
    tot_time = ttot + tdead;
    ...
    psi0 = 0;
    psif=deg2rad(20); dot_psi0= 0; dot_psif=0; ddot_psi0=0; ddot_psif=0; dddot_psi0 = 0; dddot_psif = 0;
 ...
 if (choice==2)
    %Initial and final conditions
    dot_x0= 0; dot_xf=0; ddot_x0=0; ddot_xf=0; 
    dot_y0= 0; dot_yf=0; ddot_y0=0; ddot_yf=0; 
    dot_z0= 0; dot_zf=0; ddot_z0=0; ddot_zf=0; 
    psi0 = 0;  psif=0;

    pos_0 = [x0 y0 z0];
    lin_vel_0 = [dot_x0 dot_y0 dot_z0];
    acc_0=[ddot_x0 ddot_y0 ddot_z0];
    orient_0=[0 0 0];
    w_bb_0 = [0 0 0];
    eta_0=[0 0 0];
    pos_fin=[xf yf zf];
    d0 = 3.5;           % coeff influenza
    K_attr = 0.8;
    K_rep = 0.6;
    

    
    obstacle_points_total=[];


    %uncomment for random generation of cylinder
%     for k=1:quantity;   
%      
%         obstacle_points_total=[obstacle_points_total pontis_generation(obs(k,:));];
%             
%     end
%     obstacle_points_total=obstacle_points_total';
   

    obstacle_points_total=pontis_generation(obsta_1);
    obstacle_points_total=obstacle_points_total';
